...
namespace Entities
{
    public class entUnitsTypes : BaseEntity
    {
...
_Name= string.Empty;
            ...
            : base(iID)
        {
            try
            {
                ...
                DataTable tblRow = SqlAdoWrapper.ExecuteQueryCommand("UnitsTypes_SelectByID", param,
                    false);
...
_Name= rowCurr["Name"] == DBNull.Value ?string.Empty: Convert.ToString(rowCurr["Name"]);
        ...
        public static bool operator ==(entUnitsTypes lhs, entUnitsTypes rhs)
        {
            if (lhs.ID == rhs.ID
                && lhs._Name == rhs._Name)
                return true;
